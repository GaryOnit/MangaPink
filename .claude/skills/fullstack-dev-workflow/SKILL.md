...
description: 全栈项目从零开发全流程编排器。支持从零新项目（自动初始化脚手架）和已有项目两种模式，适配前端、后端、全栈多种技术栈，无需学城账号即可使用本地存储。按需路由到需求分析→技术评审→开发→知识库同步四个阶段，并将公共参数传递给各阶段 skill，避免重复询问。触发词：帮我做个需求、开始做需求、我有个新需求、全流程开发、全流程、开始研发、走一遍流程、研发流程、从零开发、新建项目。不用于：已明确单阶段操作（单独激活对应 skill 即可）。
...
2. **SubAgent 执行原则（O-W 模式）**：每个阶段（Phase 1/2/3/4）必须通过 `task` 工具启动 SubAgent（即 Worker）执行，主 Agent（即 Orchestrator）不直接调用子 skill 的具体操作。主 Agent 的唯一职责是**调度、决策、通信**，所有阶段执行工作 100% 交由 SubAgent 完成。
...
├── 主 Agent 读 Summary → 更新 memory → 展示给用户 → 等待确认
└── [task] Phase-4 SubAgent（Worker）→ 产出 Summary
        └── [task] Phase-4 校验 Sub-SubAgent（内部一致性校验）
```
...
| `meituan-car-knowledge-base-sync` | *(待新增，读本地 frontmatter)* | 知识库同步阶段 |
...
> **目的**：在进入研发流程前，确认项目知识库三件套（AGENTS.md / docs 知识库目录 / .mrules）是否就绪。支持**从零新项目**（仓库为空）和**已有项目**两种模式，自动路由到对应处理策略。

### 步骤一：判断项目模式

首先检查用户指定的仓库路径（或当前工作区）是否存在代码文件：

```bash
# 判断是否为空仓库（无代码文件）
find . -maxdepth 3 \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \
  -o -name "*.java" -o -name "*.py" -o -name "*.go" -o -name "*.rs" \
  -o -name "*.vue" -o -name "*.swift" -o -name "*.kt" \) \
  -not -path "*/node_modules/*" -not -path "*/.git/*" 2>/dev/null | head -5
```

- **结果为空（0 个文件）** → 判定为「**从零新项目**」，进入「情况 NEW」
- **结果非空** → 判定为「**已有项目**」，进入步骤二
...
> 直接进入 Init 阶段，**不要求提前具备 AGENTS.md / .mrules / docs**。
> 在 Init 中收集技术栈信息，Phase-3 开发阶段开始时由 SubAgent 自动搭建项目脚手架并初始化 AGENTS.md 骨架。
> Phase-4 知识库同步阶段完成后，知识库将首次建立。

记录标记：`project_mode: greenfield`，静默通过，直接进入 Init 阶段。
...
[列出缺失项，每项说明用途]
  ...
  ❌ docs 知识库目录 → 需求/技术评审/开发产出文档的存放目录
...
> ⛔ **禁止**：在已有项目模式下，知识库缺失且未经用户明确选择 B/C 的情况下自动继续研发流程（AP-22）。

---

## Init：初始化问卷（必做）

进入 workflow 后，根据 Pre-Init 识别的项目模式，**一次性**向用户收集不同的信息集合。

### 从零新项目（project_mode: greenfield）收集问卷
...
2️⃣  项目根路径（必填）
    本地仓库将在此路径初始化，例如：/Users/me/projects/my-app

3️⃣  技术栈（必填，可在开发阶段补充细节）
    前端：React / Vue / Angular / Next.js / 小程序 / 其他
    后端：Node.js / Java Spring / Python / Go / 其他
    全栈：前端 + 后端（请分别说明）
    示例："前端 React + TypeScript，后端 Node.js + Express"
    如果尚不确定，可填「待定」，AI 将在技术评审阶段帮你选型

4️⃣  文档存储模式（可选，默认本地）
    A. 本地存储（产出文档写入项目 docs/ 目录）—— 默认，无需学城账号
    B. 学城目录（请提供学城目录链接或 ID）
    C. 本地 + 学城双写

5️⃣  从哪个阶段开始？（必填，请明确选择）
    A. 需求分析 → 技术评审 → 开发 → 知识库同步
    B. 技术评审 → 开发 → 知识库同步（已有需求分析结论）
    C. 仅开发 → 知识库同步（已有技术评审结论）
```
...
**解析规则**：
- 需求信息含学城链接 → `oa-skills citadel getMarkdown --contentId <id>` 读取，展示摘要确认；失败则提示用户改用文本
- `citadel_parent_id` 未提供时 → 记录 `doc_storage: local`，产出写入本地 `docs/`
- 技术栈填「待定」→ 记录 `tech_stack: TBD`，Phase-2 技术评审时 SubAgent 将主动做选型推荐
- 5️⃣（或3️⃣）未回答 → **必须追问，不得自行默认**，用户明确选择 A/B/C 后才可继续
...
需求概要：[AI 提取的 2-3 句摘要]
项目根路径：[path]
技术栈：[frontend_stack] / [backend_stack]（待定时标注）
文档存储：[本地 docs/ / 学城目录链接 / 双写]
起始阶段：[A/B/C]

确认后将：
  1. 启动研发流程（Phase-3 开发阶段将自动初始化项目目录结构和 AGENTS.md 骨架）
  2. 开发自测完成后自动进入知识库同步阶段（首次建立 docs/ 知识库）
  3. 全流程完成后创建「[项目名称] AI工作流总览」文档
...
确认后将：
  1. 启动研发流程
  2. 开发自测完成后自动进入知识库同步阶段
  3. 全流程完成后创建「[需求名称] AI工作流总览」文档

有需要调整的请告知。
```

等待用户明确批准词后：
1. 初始化 memory 快照（见「Memory 管理」章节），写入 `project_mode` / `tech_stack` / `doc_storage`
...
- project_root: <项目根路径>
- tech_stack: <frontend: xxx / backend: xxx / TBD>
- doc_storage: <local | citadel:<id> | both>
- citadel_parent_id: <id 或 N/A>
- 起始阶段: <A/B/C>
...
| Phase-4 完成后 | 更新「知识库变更」摘要和知识库同步阶段产出 |
...
公共参数：
- project_mode: <project_mode>        # greenfield（从零新项目）| existing（已有项目）
- project_root: <project_root>        # 项目根路径
- tech_stack: <tech_stack>            # 例：frontend: React+TypeScript / backend: Node.js+Express / TBD
- doc_storage: <doc_storage>          # local | citadel:<id> | both
- citadel_parent_id: <citadel_parent_id>  # 学城目录 ID，doc_storage=local 时为 N/A
- 需求名称: <需求名称>
...
【步骤 0 — 仅 project_mode=greenfield 时执行：脚手架初始化】
0.1 根据 tech_stack 判断项目类型并初始化目录结构：
    · 纯前端（React/Vue/Next.js 等）：
      ```bash
      cd <project_root>
      # 根据技术栈选择合适的脚手架命令，例如：
      # React+TypeScript: npx create-react-app . --template typescript
      # Next.js: npx create-next-app@latest . --typescript --tailwind --app
      # Vue3: npm create vue@latest .
      ```
    · 纯后端（Node.js）：
      ```bash
      mkdir -p src test && npm init -y
      # 根据框架安装依赖，例如 Express: npm install express && npm install -D typescript ts-node @types/node @types/express
      ```
    · 纯后端（Java Spring）：使用 Spring Initializr 或 Maven archetype 生成骨架
    · 纯后端（Python）：`mkdir -p src tests && python -m venv venv && pip install ...`
    · 全栈：按照上述规则分别初始化 frontend/ 和 backend/ 子目录
    · tech_stack=TBD：根据需求和技术评审结论自动选择最合适的技术栈，**输出 CHECKPOINT PENDING 前告知用户选型决策及理由**
0.2 在 <project_root> 根目录生成 AGENTS.md 骨架文件（AI 编码约束文件）：
    ```markdown
    # AGENTS.md — [项目名称]
    > ⚠️ 本文件由 AI 研发流程自动生成，开发过程中持续更新。
    ## 项目概述
    [1-2句项目描述]
    ## 技术栈
    - 前端：[stack]
    ...
    [自动生成的目录树]
    ## 编码约束
    - [根据技术栈补充，如 TypeScript strict mode、ESLint 规则等]
    ## 已知坑（开发过程中持续追加）
    ```
0.3 初始化 Git 仓库（若不存在）：
    ```bash
    ...
    git init
    echo "node_modules/\n.env\ndist/\nbuild/\n.catpaw/" >> .gitignore
    git add . && git commit -m "chore: init project scaffold via AI workflow"
    ```
0.4 输出 CHECKPOINT 0 PENDING（脚手架确认），等待用户确认后继续步骤 1
...
3. 交叉验证：每个 CheckPoint 编码完成后，创建一个 Sub-SubAgent（task 工具，subagent_type: general-agent）进行代码评审，检查：
   · 是否遵守项目编码规范（读取 AGENTS.md / .mrules，greenfield 模式下读取刚生成的 AGENTS.md 骨架）
   · 是否有明显的逻辑错误或遗漏
   · 接口签名是否与技术评审一致
   Sub-SubAgent 的评审意见必须在修正代码后体现
4. 每完成一个 CheckPoint（包括 CheckPoint 0 脚手架确认 / CheckPoint 1 工作量评估），立即输出 CHECKPOINT PENDING 格式后停止
5. 所有 CheckPoint 用户均确认后，产出代码评审文档 + 测试用例文档；doc_storage=local 时写入 <project_root>/docs/，doc_storage=citadel 时写入 citadel_parent_id，doc_storage=both 时双写
6. ⛔ 禁止"CheckPoint 间的用户确认由本 SubAgent 直接向用户请求"——SubAgent 无此能力，必须通过 PENDING + resume 机制
7. 文档目录已由 workflow 提供（doc_storage），不得再次询问用户
...
- 现有知识库: AGENTS.md + docs/

执行要求：
1. 读取 `meituan-car-knowledge-base-sync` skill（待新增；若本地不存在，必须在 Summary 中明确标注并停止）
2. 基于本次开发结果对现有知识库做增量同步：该新增的新增、该更新的更新、该失效的归档或删除
3. 若 `docs/README.md`、`docs/service-api-index.md`、`docs/quick-lookup.md`、`AGENTS.md` 受影响，优先同步这些索引型文档
4. 对失效知识默认优先归档而非直接物理删除；若确需删除，必须在 Summary 中单列删除原因和影响范围
5. 完成后输出知识库更新说明文档到 citadel_parent_id，并输出标准 Summary
6. 学城目录已由 workflow 提供，不得再次询问用户

最终 Summary 格式：
=== PHASE 4 SUMMARY ===
状态：✅ 完成 / ⚠️ 完成（有问题） / ❌ 失败
产出文档：
  · 知识库更新说明 → https://km.sankuai.com/collabpage/[id]
知识库变更：
  · 新增: [N] 项
  · 更新: [N] 项
  · 归档/删除: [N] 项
...
| 知识库同步 | [开发阶段摘要 + 分支/提交] | [知识库更新说明 或 已跳过] | [用户问题] |
...
| Phase-4 知识库同步失败 | 不得创建总览文档；先向用户说明失败原因，再决定重试、跳过或改为人工处理 |
...
- **知识库同步是开发后的固定后置阶段**：Phase-3 确认后先进入 Phase-4，再允许创建总览文档
...
| AP-15 | 主 Agent 收到 PHASE 1/2 PENDING 后自动 resume 不等用户确认 | 与 AP-13 相同原则，所有阶段均不可自动推进 |
...
| AP-23 | 开发阶段结束后跳过知识库同步，直接输出全流程汇总 | 会导致知识库与实际代码状态脱节，总览信息也不完整 |
| AP-24 | 主 Agent 直接顺手修改 `AGENTS.md` 或 `docs/`，不走 Phase-4 SubAgent | 知识库同步属于独立阶段，必须通过 Worker 执行并产出 Summary |
| AP-25 | 在 Phase-4 中未说明依据就直接物理删除知识库文档 | 删除风险高，至少应先归档或在 Summary 中显式说明删除原因和影响范围 |